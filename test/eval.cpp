...
int main()
{
    EM_EVAL(
        ...
        (
            EM_EVAL_SET_A( std::max EM_P( EM_EVAL_B, EM_EVAL_A )  )
        )
        std::cout << EM_EVAL_A << '\n';
    )
    ...
    EM_EVAL(
        ...
        (
            [[maybe_unused]] int EM_VA_AT0 EM_P(EM_EVAL_A) = EM_VA_AT1 EM_P(EM_EVAL_A);
            EM_EVAL_FOREACH_B( (10)(20)(30) )
            (
                std::cout << EM_VA_AT1 EM_P(EM_EVAL_A) * EM_EVAL_B << '\n';
                EM_EVAL_SET_C( EM_EVAL_C + EM_VA_AT1 EM_LP EM_EVAL_A EM_RP * EM_EVAL_B )
            )
        )
